...
from awesome.model.real_nvp.coupling_layer_1d import CouplingLayer1D, MaskType
...
class _RealNVP(nn.Module):
    ...
    def __init__(self, scale_idx, num_scales, in_channels, mid_channels, num_blocks):
        ...
        self.in_couplings = nn.ModuleList([
            CouplingLayer1D(in_channels, mid_channels, num_blocks, MaskType.CHECKERBOARD, reverse_mask=False),
            CouplingLayer1D(in_channels, mid_channels, num_blocks, MaskType.CHECKERBOARD, reverse_mask=True),
            CouplingLayer1D(in_channels, mid_channels, num_blocks, MaskType.CHECKERBOARD, reverse_mask=False)
        ])
        ...
        if self.is_last_block:
            ...
            self.out_couplings = nn.ModuleList([
                CouplingLayer1D(4 * in_channels, 2 * mid_channels, num_blocks, MaskType.CHANNEL_WISE, reverse_mask=False),
                CouplingLayer1D(4 * in_channels, 2 * mid_channels, num_blocks, MaskType.CHANNEL_WISE, reverse_mask=True),
                CouplingLayer1D(4 * in_channels, 2 * mid_channels, num_blocks, MaskType.CHANNEL_WISE, reverse_mask=False)
            ])
