...
class TruePixelAccuracy(TorchMetric):
    """Calculates the pixel accuracy but only these which are trues in the target, e.g. percentage of correctly predicted pixels."""
    ...
    def __call__(self, output: VEC_TYPE, target: VEC_TYPE, **kwargs) -> torch.Tensor:
        ...
        if self.noneclass is not None:
            ...
            target = target[mask]
            output = output[mask]
        _len = target.float().sum() # Count only the trues 
        bin = (output == target).float()
        bin = bin[target.float() == 1].float() # Count only the trues
        if _len == 0.:
